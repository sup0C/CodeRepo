...
def train(model, optimizer, train_loader, epoch):
    ...
    for batch_id, (data, target) in enumerate(tqdm(train_loader)):
        # torch.eye是单位矩阵
        target = torch.sparse.torch.eye(10).index_select(dim=0, index=target) # (32,10)
        data, target = Variable(data), Variable(target)

        if USE_CUDA:
            data, target = data.cuda(), target.cuda() # (32,1,28,28)

        optimizer.zero_grad()
        #  (32,10,16,1)、 (32,1,28,28)、(32,10)=（32，cls_num）
...
if __name__ == '__main__':
    ...
    capsule_net = CapsNet(config) # 网络
    capsule_net = torch.nn.DataParallel(capsule_net)
    if USE_CUDA:
        capsule_net = capsule_net.cuda()
    capsule_net = capsule_net.module

    optimizer = torch.optim.Adam(capsule_net.parameters()) # 优化器
