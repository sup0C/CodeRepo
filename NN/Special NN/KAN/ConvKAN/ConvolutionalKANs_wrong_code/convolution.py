...
def calc_out_dims(matrix, kernel_side, stride, dilation, padding):
    ...
    计算输入的图片在卷积之后的w、h的尺寸
    Args:
        matrix:输入的图片(batch_size,c,h,w)
        kernel_side:卷积核大小，也即是kernel_size，这里可能是打错字了。
        stride:
        dilation:
        padding:
    Returns:输出为卷积之后图片的(batch_size,c,h,w)，即h_out,w_out,batch_size,n_channels
    '''
...
def multiple_convs_kan_conv2d(matrix, #but as torch tensors. Kernel side asume q el kernel es cuadrado
             kernels, kernel_side,out_channels,stride= (1, 1),
             dilation= (1, 1), padding= (0, 0),device= "cuda"
             ) -> torch.Tensor:
    """
    Makes a 2D convolution with the kernel over matrix using defined stride, dilation and padding along axes.
    进行2维卷积
    Args:
        matrix (batch_size, colors, n, m]): 2D matrix to be convolved.也就是输入的图片(batch_size,c,h,w)
        kernel_side：卷积核大小，也即是kernel_size，这里可能是打错字了。
