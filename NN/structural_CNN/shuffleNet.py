...
def shuffle_channels(x,groups):
    ...
    assert channels%groups==0  # 断言函数：对表达式布尔值的判断，要求其计算值必须为真，若表达式为假，触发异常；若为真，不报错。
    channels_per_group=channels//groups
    # split into groups: (b, num_channels, h, w)  -->  b, groups, channels_per_group, h, w
    x=x.view(batch_size,groups,channels_per_group,height,width)
    # transpose - channelshuffle
...
class ShuffleNetUnitA(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, groups=4):
        ...
        bottleneck_channels = out_channels // groups
        self.groups = groups
