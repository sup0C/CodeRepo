...
def data_extraction():
    """节点数据抽取
    返回抽取并去重的买方、卖方和抽取的了excel中所有的信息
    """

    # 取出购买方名称到list
    node_buy_key = []
    for i in range(0, len(invoice_data)):  # len(invoice_data)是行长
        # 遍历数据
    ...
    # value抽出作node，相当于抽取了excel中所有的信息- 这段代码的信息本案例没有用
    node_list_value = []
    for i in range(0, len(invoice_data)):
...
def relation_extraction():
    ...
    for i in range(0, len(invoice_data)):
        # 遍历数据,获取excel数据中每行数据中买方、卖方和金额三方的属性值
...
relation_extraction() # 抽取关系
create_data = DataToNeo4j() # 创建图

create_data.create_node(data_extraction()[0], data_extraction()[1]) # 创建节点
create_data.create_relation(relation_extraction()) # 创建关系