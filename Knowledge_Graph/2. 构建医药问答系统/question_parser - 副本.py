class QuestionPaser:
    def build_entitydict(self, args):
        '''
        构建实体节点
        :param args:从用户问题中提取出的关键词和其类型的映射词典，如{'感冒': ['disease']}
        :return:输出是一个dict，是类型往实体的映射，即问题中在该类型的特征词有哪些，如{'disease': ['感冒']}
        '''
        entity_dict = {}
    ...
    def parser_main(self, res_classify):
        '''
        解析主函数
        :param res_classify:dict。 里面有两个元素('args'和'question_types'），'args'是medical_dict，即从用户问题中提取出的关键词和其类型的映射词典，如{'感冒': ['disease']}
        另一个是question_types,主要是用户的该问题属于哪种类型问题的映射表，如disease_symptom、'disease_do_food'等
        :return:list。里面有N个词典，每个词典有两个元素，分别是question_type和目标执行的N条cql语句
        ...
        entity_dict = self.build_entitydict(args) # dict，是类型往实体的映射，即问题中在该类型的特征词有哪些，如{'disease': ['感冒']}
    ...
    def sql_transfer(self, question_type, entities):
        '''
        针对问题，根据其所属的多个问题类型分开进行处理
        :param question_type:用户问题的焦点，即'disease_symptom'
        :param entities:dict，即问题在该类型中被匹配到的特征词有哪些，如['感冒']
        :return:返回在图中进行查询的cql语句
        '''
        if not entities:
            return []
        # 查询语句
