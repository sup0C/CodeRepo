import os
import ahocorasick # 匹配算法。一种用于精确或近似多模式字符串搜索的高效算法
...
class QuestionClassifier:
    def __init__(self):
        ...
        self.deny_path = os.path.join(cur_dir, 'dict/deny.txt') # 否定词
        # 加载特征词
        ...
        # set 集合中的元素是无序的，self.region_words中是所有节点实体的名称
    ...
    def classify(self, question):
        ...
        分类主函数
        :param question:问句
        :return:字典，面有两个元素('args'和'question_types'），'args'是medical_dict，即从用户问题中提取出的关键词和其类型的映射词典，如{'感冒': ['disease']}
        另一个是question_types,主要是用户的该问题属于哪种类型问题的映射表，如disease_symptom、'disease_do_food'等
        '''
        data = {}
        medical_dict = self.check_medical(question) # medical_dict是{'感冒': ['disease']}
        if not medical_dict:
            return {}
        ...
        question_types = []
        # 从问句中提取出了N个实体类型，如'disease'、'symptom'，同时如果问句中存在某个实体类型域C中的特征词c_1，
        # 则认为该问题属于某种类型问题（question_type），一个问题可以有多种question_type，
        # 如disease_symptom、'disease_do_food'等。
        # 即判断用户在问什么，如药物？症状？恢复时间等等。
        if self.check_words(self.symptom_qwds, question) and ('disease' in types):
            question_type = 'disease_symptom'
            question_types.append(question_type)
        # 根据症状推断疾病，即针对问题“有恶心的症状是什么原因?”
    ...
    def build_wdtype_dict(self):
        '''构造词对应的类型
        输出一个词典，里面是所有实体所对应的标签。即key(每个实体名称):value(该实体所属标签)
        '''
        wd_dict = dict()
    ...
    def build_actree(self, wordlist):
        '''
        基于AC算法构造actree，加速过滤
        :param wordlist:输入为self.region_words，其中是所有节点实体的名称
        :return:返回
        '''
        actree = ahocorasick.Automaton() # AC自动机
        for index, word in enumerate(wordlist):
            actree.add_word(word, (index, word)) # 将字符串键及其关联值添加到自动机，可作为 trie 树使用
        actree.make_automaton() # 完成并创建 Aho-Corasick 自动机
        return actree
    ...
    def check_medical(self, question):
        ...
        :return:返回dict。是匹配到的词和该词的标签
        '''
        region_wds = []
        # 开始查找，iter方法匹配所有字符串
        # iter(string, (start, (end)))：使用提供的输入字符串执行 Aho-Corasick 搜索过程。
        # 它返回一个迭代器，为在字符串中找到的键返回元组 (end_index, value)，
        # 其中 end_index 是匹配结束的索引位置，value 是与匹配的键相关联的值。
        ...
        final_dict = {i:self.wdtype_dict.get(i) for i in final_wds} # dict。匹配到的词和该词的标签
