...
class StoDepth_BasicBlock(nn.Module):
    ...
    def __init__(self, prob, multFlag, inplanes, planes, stride=1, downsample=None):
        ...
        self.multFlag = multFlag # bool。default：false。
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            if torch.equal(self.m.sample(), torch.ones(1)):
                ...
                out += identity
            #为0时，只剩下identity，且卷积部分的参数不进行梯度更新
            else:
        ...
        #测试推理阶段激活所有网络层
            ...
            if self.downsample is not None:
                identity = self.downsample(x)
            #以概率prob整合网络，这里对这种整合进行可配置化
            if self.multFlag:
                # 原始论文在测试中使用了以下公式，不过有时这会导致性能下降
                # 如果你不想在测试结果上乘概率，可以把multFlag改为False。
...
if __name__ == '__main__':
    ...
    model = resnet18_StoDepth_lineardecay(pretrained=False)